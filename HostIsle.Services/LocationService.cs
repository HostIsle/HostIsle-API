...
namespace HostIsle.Services
{
    ...
    public class LocationService : ILocationService
    {
        ...
        {
            var addresses = (await this.addressRepo.GetAllAsync()).Select(a => a.Description).ToList();

            Address address;

            if (addresses.Contains(model.AddressText))
            {
                address = (await this.addressRepo.GetAllAsync()).FirstOrDefault(a => a.Description == model.AddressText);
            }
            else
            {
                var towns = (await this.townRepo.GetAllAsync()).Select(t => t.Name).ToList();

                Town town;

                if (towns.Contains(model.TownName))
                {
                    town = (await this.townRepo.GetAllAsync()).FirstOrDefault(t => t.Name == model.TownName);
                }
                else
                {
